...
# Each list represents a stack and each item in list represents the crates 
# Solution:
# Given the stacks and a file with instructions on how to move the crates, determine the last crate in each stack
# Open the file for reading
# Parse each line to determine how many crates to move, the starting stack and the end stack
# split each line into a list, store the value for the number of crates
# use the pop() function to move crates one at a time into the required stacks
...
with open (file_name, "r") as file:
    for line in file:
        ...
        while move > 0:
            find_crate(end_crate).append(find_crate(start_crate).pop())
            move -= 1
